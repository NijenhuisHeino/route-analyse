window.postnlFastMap = (() => {
  let map;
  let abortController;
  let dotNetRef;

  function apiUrl(path) {
    const prefix = window.location.pathname.startsWith("/fast") ? "/fast/api/" : "/api/";
  ...
  return {
    ...
    update: async (filter, chargerFilter, overnightFilter, options) => {
      if (!map) return;
      if (!isStyleReady()) {
        window.setTimeout(() => window.postnlFastMap.update(filter, chargerFilter, overnightFilter, options), 100);
        return;
      }
      addLayers();

...
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), "postnl-fast-tests", Guid.NewGuid().ToString("N"));
    ...
    public RouteAnalysisServiceTests()
    {
        ...
        var warmup = new FastCacheWarmupService(store, NullLogger<FastCacheWarmupService>.Instance);
